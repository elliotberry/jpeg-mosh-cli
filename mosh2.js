...
async function mosh_jpeg_data(jpegdata, typ = 3, qt = [2, 1], im = [15, 1], validate = false, validate_maxtries = 10) {
  ...
  while (tries > 0) {
    tries--;
    const ret = [];
    const segments = await read_structure(jpegdata);

    for (const segment of segments) {
      if (segment.marker === DQT && typ & 1) {
        const corruptedSegdata = flipbits(segment.segdata, qt[0], qt[1], 4);
        ret.push(corruptedSegdata);
      } else if (segment.marker === SOS && typ & 2) {
        const corruptedSegdata = flipbits(segment.segdata, im[0], im[1], 20);
        ret.push(corruptedSegdata);
      } else {
        ret.push(segment.segdata);
      }
    }
